  ...
  await knex.transaction(async (trx) => {
    ...
    if (users.length > 0) {
      ...
      if (identityProviderUsers.length > 0) {
        await knex.batchInsert('identity_provider_user', identityProviderUsers, 100).transacting(trx);
      }

      const sessions = await trx('session').withSchema('v1').whereIn('user_id', whereInUserIds);

      if (sessions.length > 0) {
        await knex.batchInsert('session', sessions, 100).transacting(trx);
      }
    }
    ...
    if (projectManagers.length > 0) {
      await knex.batchInsert('project_manager', projectManagers, 100).transacting(trx);
    }
    ...
    if (labels.length > 0) {
      await knex.batchInsert('label', labels, 100).transacting(trx);
    }
    ...
    if (cardSubscriptions.length > 0) {
      await knex.batchInsert('card_subscription', cardSubscriptions, 100).transacting(trx);
    }
    ...
    if (cardMemberships.length > 0) {
      await knex.batchInsert('card_membership', cardMemberships, 100).transacting(trx);
    }

    const cardLabels = await trx('card_label').withSchema('v1').whereIn('card_id', whereInCardIds);

    if (cardLabels.length > 0) {
      await knex.batchInsert('card_label', cardLabels, 100).transacting(trx);
    }
